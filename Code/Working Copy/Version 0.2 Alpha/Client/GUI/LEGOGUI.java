...
public class LEGOGUI extends JFrame {
    /********************** Map GUI Panel component **************/
     ...
     private NoGoZoneListener ngz = new NoGoZoneListener();
     NewMapStart nms;
    ...
    public void reInitialMap(){
     ...
     public class MapCanvas extends Canvas{
        ...
        public void paint(Graphics g) {
           ...
           {// set map bounds;
            g.setColor(Color.black);
            g.drawRect((int)(mapX*mapProportion),(int)(mapY*mapProportion), (int)(width*mapProportion), (int)(height*mapProportion));
           }
           // SET No go zone
           		if(NoGoSetting){
            ...
            g.setColor(Color.green);
           	g.fillRect(mapX + map.getCurrentPixel().getxPos() * mapAlter + 1,mapY + map.getCurrentPixel().getyPos()* mapAlter + 1, mapAlter - 1, mapAlter - 1);
         ...
         public class ConfirmListener implements ActionListener {
            public  void actionPerformed(ActionEvent e) { 
               ...
               if(label.equals("Confirm")){
                    ...
                    RobotDirection = 2;
                    map = new Map(mapId,height/mapAlter,width/mapAlter,Integer.parseInt(X_Value.getText())/mapAlter,Integer.parseInt(Y_Value.getText())/mapAlter,RobotDirection);
    ...
    class NewMapAction implements ActionListener {
        ...
        public void NoGoZoneInit() {
        	 ...
        	 if(response==0){ 
        		 canvas.addMouseListener(ngz);
             	 NoGo = new int[4][2];
             	 NoGoCounter = 0;
             	 NoGoCir = 0;
                 append("Start Setting No-go Zone:");
             }else{
                 append("No-go Zone Setting Canceled.");
             }
    ...
    public void NoGoZoneClose(){
    	...
    	for(int i =0; i < 4; i++){
    		tempX[i] = NoGo[i][0];
    		tempY[i] = NoGo[i][1];
    		//System.out.print(" (i:"+ tempX[i] + " j:" + tempY[i] +")");
    	}
    	s = new Polygon(tempX,tempY,4); 
    	for(int i = tempRec[0]; i < tempRec[2]; i ++ )
    		for(int j = tempRec[1] ; j < tempRec[3] ; j ++)
    			if(s.contains(i,j))    	
    	        	map.findPixel((int)((i - mapX)/mapAlter), (int)((j - mapY)/mapAlter)).setValue(NOGOZONE);
    	          		    	
    	NoGoSetting =false;
    	NoGoCounter = 0;
    	NoGo = null;
    	MapRepaint();
    	canvas.removeMouseListener(ngz);
    }
    public void sort(){
    	tempRec[0] = NoGo[0][0];
		tempRec[1] = NoGo[0][1];
    	...
    	//System.out.println("MinX:" + tempRec[0]+" MinY:" + tempRec[1] + " MaxX:" + tempRec[2] + " MaxY:"+tempRec[3]);
    }
    ...
    public class NavigationListener implements ActionListener {
        public  void actionPerformed(ActionEvent e) { 
        	rn.update();
        	AutoNavigation an = new AutoNavigation(map,mct,rn);
        	an.start();
    ...
    public class ManualScanListener implements ActionListener {
        public  void actionPerformed(ActionEvent e) { 
            ...
            if(response ==0){
            	rn.update();
            	ManualNavigation mn = new ManualNavigation(map,mct,rn);
            	mn.start();
   ...
   public void LoadMap(){
    ...
    public class NoGoZoneListener implements MouseListener{
        ...
        public void mouseClicked(MouseEvent e){
        	...
        	if(!outOfMap(x,y)){
        		if(NoGoCounter < 4){
        			...
        			NoGoCounter ++;
        			append("Point " + NoGoCounter + ": >>>>  X: " + NoGo[NoGoCounter-1][0] + " Y: " + NoGo[NoGoCounter-1][1]);
                	MapRepaint();
        		}
        	}
        	else{
        ...
        public boolean outOfMap(int x, int y){
        	if(x < (int) (mapX*mapProportion) || x > (int)((mapX + width)*mapProportion) || y < (int)(mapY*mapProportion) || y > (int)((mapY + height)*mapProportion))
        		return true;
        	else
        		return false;       	
        } 
