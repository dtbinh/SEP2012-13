...
public class RobotNavigator {
	...
	/**
	 * Start navigation from current pixel.
	 * @return pixel which robot stopped at
	 */
	public static Pixel startNavigation(){
		System.out.println("Starting Navigation..");
		while(!mct.isReady() && !mct.isReadyForNextCommand){
			try {
				Thread.sleep(200);
		...
		while(myMap.getCurrentPixel().hasNextPixel()){
			System.out.println("Robot Current At: "+ myMap.getCurrentPixel());
			if(myMap.getCurrentPixel().getValue() == Pixel.UNEXPLORED || myMap.getCurrentPixel().getValue() == Pixel.BUFFERZONE){
				getGroundReadings();
	...
	/**
	 * Check and update ground information
	 */
	 ...
	 * @return true no walls are detected 
	 */
	private static boolean checkAndUpdateWalls() {
		...
		if( wallReading[0] < 28 ){
			...
			if(wallReading[0] >= 5 && wallReading[0] < 8){
				...
				vector = 8;
			}else if(wallReading[0] >= 24 && wallReading[0] < 27){
				vector = 9;
			}
		...
		if( wallReading[1] < 27 ){
			...
			if(wallReading[1] >= 8 && wallReading[1] < 12){
				...
				vector = 7;
			}else if(wallReading[1] >= 23 && wallReading[1] <= 26){
				vector = 8;
			}
		...
		if( wallReading[2] < 24 ){
			...
			if(wallReading[2] >= 7 && wallReading[2] < 11){
				...
				vector = 8;
			}else if(wallReading[2] >= 23 && wallReading[2] < 26){
				vector = 9;
			}
			if(vector != 0)
				switch(myMap.getRs().getRobotDirection()){
	...
	/**
	 * Send command and get ground informations of current pixel.
	 */
	private static void getGroundReadings() {
		while(!mct.isReadyForNextCommand && !mct.isReady()){
			try {
				Thread.sleep(200);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
	...
	/**
	 * Send command and get wall informations of current pixel.
	 */
	...
	/**
	 * goes to next pixel if there is one
	 */
	...
	/**
	 *	Tell the robot to go to pixel behind robot.
	 */
	...
	/**
	 * Tell robot to go to the pixel on the right.
	 */
	...
	/**
	 * Tell robot to go to the pixel on the left.
	 */
	...
	/**
	 * Tell robot to go to the pixel to the front.
	 */
